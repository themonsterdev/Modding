...
void Log::Init(HMODULE hModule) {
	...
	if (GetModuleFileNameA(hModule, g_logFile, MAX_PATH) != 0) {
		...
		sprintf_s(chLogBuff, "INIT: MenuBaseDLL Initialisation\n");

		if (slash != -1) {
			g_logFile[slash + 1] = '\0';
			strcpy_s(g_debugLogFile, g_logFile);

			strcat_s(g_debugLogFile, "MenuBaseDLL.log");
			...
			OutputDebugStringA("INIT: PurplePower");
		}
		else {
			useFileOutput = false;
		}
...
void Log::Msg_Simple(const char* Path, const char* fmt, ...)
{
	va_list va_alist;
	char chLogBuff[CHARS_FOR_BUFF];
	char chParameters[CHARS_FOR_PARAMS];
